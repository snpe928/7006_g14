...
if uploaded_file is not None:
    selected_claim_number = st.number_input('Select Claim Number', input_df['claim_number'])
...
with tab1:
    ...
    ChangeofAddress = row['Change of Address'].iloc[0]
    LivingStatus = row['Living Status'].iloc[0]
    HighEducation = row['High Education'].iloc[0]
    Witness = row['Witness'].iloc[0]
    PoliceReport = row['Police Report'].iloc[0]
    MaritalStatus = row['Marital Status'].iloc[0]
    ClaimDay = row['Claim Day'].iloc[0]
    AccidentSite = row['Accident Site'].iloc[0]
    AnnualIncome = row['Annual Income'].iloc[0]
    ClaimAmount = row['Claim Amount'].iloc[0]
    PastNumberofClaims = row['Past Number of Claims'].iloc[0]
    AgeofVehicle = row['Age of Vehicle'].iloc[0]
...
with tab2:
    ...
    if uploaded_file is None:
        st.error("Please upload a CSV file to get started. If you have already uploaded a file, please select the file from the left sidebar.")
    else:
        ...
        numerical_features = ['Annual Income', 'Safety Rating','Past Number of Claims', 'Liability (%)', 'Claim Amount', 'Age of Vehicle']
        #numerical_features = input_df.select_dtypes(include='number').columns.tolist()
        #numerical_features.remove('claim_number')
        #numerical_features.remove('zip_code')
        #numerical_features.remove('claim_date')
        #numerical_features.remove('vehicle_price')
        #numerical_features.remove('vehicle_weight')
        #numerical_features.remove('fraud')
        ...
        categorical_features = ['Gender', 'Marital Status', 'High Education', 'Change of Address', 'Living Status',
                            'Claim Day', 'Accident Site', 'Witness',
                            'Police Report']
        ...
        input_df.drop(['claim_date','vehicle_price', 'vehicle_weight','channel', 'vehicle_category','vehicle_color', 'claim_number', 'zip_code', 'age_of_driver'], inplace = True, axis = 1)
        # Label encoding
        input_df.replace({'Gender': {'M': '0', 'F': '1'},
                'Living Status': {'Rent': '0', 'Own': '1'},
                'channel': {'Broker': '0', 'Online': '1', 'Phone': '2'},
                'vehicle_category': {'Compact': '0', 'Large': '1', 'Medium': '2'},
                'Accident Site': {'Local': '0', 'Highway': '1', 'Parking Lot': '2'},
                'vehicle_color': {'white': '0', 'silver': '1', 'gray': '2', 'black': '3', 'red': '4', 'blue': '5', 'other': '6'},
                'Claim Day': {'Monday': '0', 'Tuesday': '1', 'Wednesday': '2', 'Thursday': '3', 'Friday': '4', 'Saturday': '5', 'Sunday': '6'}}, inplace=True)
